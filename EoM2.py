...
def deriv2(allvar,time, params):
    ...
    for i in range(1,nM):
        ...
        d[3*i + 2] = np.divide(9*(((i+0.5)**(4/3.0))*(variables[3*i+5]-variables[3*i+2]) -((i-0.5)**(4/3.0))*(variables[3*i+2] - variables[3*i-1]))*params[2],(params[-1])**(2/3.0)) + np.divide(params[4]*variables[3*i+1],1+variables[3*i+1])
    ...
    d[2] = np.divide(18*(0.5**(4/3.0))*(variables[5] - variables[2])*params[2], (params[-1])**(2/3.0)) + np.divide(params[4]*variables[1],1+variables[1])
